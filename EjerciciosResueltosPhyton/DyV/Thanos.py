...
m = int(input()) #Numero de amoches producidos
identHabAmochados = list(map(int, input().strip().split()))
identHabAmochados.sort()
p = int(input()) #Número de habitantes que vamos a comprobar
identHabComprobar = list(map(int, input().strip().split()))

for i in identHabComprobar:
    index = bynary_Search(i, identHabAmochados)
    if index >= 0:
        index2 = bynary_Search(i, identHab)
        if index2 >= 0:
            print(":_(")
        else:
            print(":)")
